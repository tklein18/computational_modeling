...
to go ; this is the master schedule
  ...
  if ticks >= 1000 [
    ...
    export-plot "Corridor Width"
      (word "real_results/Corridor-output-for-q-" selected_q_value ".csv")
    stop
  ]
end
...
## Initialization
The topography of the landscape (the elevation of each patch) is initialized when the model starts. The topography is of a real study site, imported from a file containing elevation values for each patch. 

## Input Data
The model imports data from a text file that has elevation data for each patch.
